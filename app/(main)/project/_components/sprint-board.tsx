...
import { Issue, IssueStatus, Sprint } from "@/types/schema";

interface ISprintBoardProps {
  sprints: any[];
  projectId: string;
  orgId: string;
}
...
export default function SprintBoard({ sprints, projectId, orgId }: ISprintBoardProps) {
  const [currentSprint, setCurrentSprint] = useState(sprints.find((spr) => spr.status === "ACTIVE") || sprints[0]);

  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  const [selectedStatus, setSelectedStatus] = useState<string | null>(null);

  const { loading: issuesLoading, error: issuesError, fn: fetchIssues, data: issues, setData: setIssues }: any = useFetch(getIssuesForSprint);
  ...
  const { fn: updateIssueOrderFn, loading: updateIssuesLoading, error: updateIssuesError }: any = useFetch(updateIssueOrder);
  ...
  const onDragEnd = async (result: any) => {
    ...
    if (source.droppableId === destination.droppableId) {
      const reorderedCards: any = reorder(sourceList, source.index, destination.index);
