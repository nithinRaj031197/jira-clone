...
export async function getOrganization(slug: string): Promise<any | null> {
  const { userId } = auth();
  if (!userId) {
...
export async function getOrganizationUsers(orgId: string) {
  ...
  const userIds: any = organizationMemberships.data.map((membership) => membership?.publicUserData?.userId);
